...
#ifndef RubiksCube_IDAStar_Solver_h
#define RubiksCube_IDAStar_Solver_h

template<typename T, typename H>
class IDAStar_EdgeSolver {
private:
...
//        priority_queue contains pair(Node, move done to reach that)
        ...
        Node start = Node(rubiksCube, 0, EdgeDB.getNumMoves(rubiksCube));
        ...
        while (!pq.empty()) {
            ...
            for (int i = 0; i < 18; i++) {
                ...
                if (!visited[node.cube]) {
                    node.estimate = EdgeDB.getNumMoves(node.cube);
...
public:
    ...
    IDAStar_EdgeSolver(T _rubiksCube, string fileName) {
        rubiksCube = _rubiksCube;
        EdgeDB.fromFile(fileName);
    }
...
#endif  //RubiksCube_IDAStar_Solver_h